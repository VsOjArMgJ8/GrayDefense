...
class MLclf():
    root_dir = "/home/xfLee/Datasets"
    download_dir = root_dir + '/MiniImagenet'
    ...
    download_dir_tinyIN = root_dir + '/TinyImagenet'
    ...
    def miniimagenet_download(Download=False):
        ...
        if Download:
            ...
            if not os.path.isdir(MLclf.download_dir + '/miniimagenet'):
                ...
                if os.path.isfile(MLclf.download_dir + '/miniimagenet' + '/mini-imagenet-cache-train.pkl') and os.path.isfile(MLclf.download_dir + '/miniimagenet' + '/mini-imagenet-cache-val.pkl') and os.path.isfile(
                        MLclf.download_dir + '/miniimagenet' + '/mini-imagenet-cache-test.pkl'):
    ...
    def miniimagenet_clf_dataset(data_dir=None, ratio_train=0.6, ratio_val=0.2, seed_value=None, shuffle=True, save_clf_data=True, transform=None):
        data_feature_label_permutation_split = MLclf.miniimagenet_convert2classification(data_dir=data_dir, ratio_train=ratio_train, ratio_val=ratio_val, seed_value=seed_value, shuffle=shuffle, task_type='classical_or_meta',
                                                                                         save_clf_data=save_clf_data, transform=transform)
    ...
    def tinyimagenet_convert2classification(data_dir=None, ratio_train=0.6, ratio_val=0.2, seed_value=None,
        ...
        data_load_train = [images_train, labels_train]  # list for the values of the dict.
        data_load_val = [images_val, labels_val]
        ...
        if task_type == 'raw':
            ...
            return data_load_train, data_load_val
        """
        n_samples_train = data_load_train['image_data'].shape[0]
        n_samples_val = data_load_val['image_data'].shape[0]
        n_samples_test = data_load_test['image_data'].shape[0]
    ...
    def tinyimagenet_data_raw(data_dir=None):
        data_raw_train, data_raw_val = MLclf.tinyimagenet_convert2classification(data_dir=data_dir,
                                                                                                task_type='raw')
        return data_raw_train, data_raw_val
