...
# from utils_.models_vit import ViTBase16
from utils_.models_vit import ViTBase32

# 1.定义模型，并使用后门模型参数进行初始化
# model = ViTBase16(param.classes_num, True)
# relearn_model = ViTBase16(param.classes_num, True)
model = ViTBase32(param.classes_num, True)
relearn_model = ViTBase32(param.classes_num, True)
...
train_set, _ = split(train_set, 0.15)
...
relearn_optimizer = torch.optim.AdamW(
    list(relearn_model.vit.encoder.layers[-1].parameters()) + list(relearn_model.vit.encoder.layers[-2].parameters())
    + list(relearn_model.vit.heads.parameters()) + list(relearn_model.vit.encoder.ln.parameters())
    , lr=0.0001
)
...
def register_hooks():
    ...
    handle = relearn_model.vit.encoder.layers[-2].register_forward_hook(save_feature)
    ...
    handle = model.vit.encoder.layers[-2].register_forward_hook(save_ref)
    hook_handles.append(handle)
...
def relearning(epochs):
    ...
    for epoch in range(epochs):
        ...
        for index, (inputs, target) in enumerate(train_loader):
            ...
            feature_map_min = feature.min(dim=-1, keepdim=True)[0]
            feature_map_max = feature.max(dim=-1, keepdim=True)[0]
            ...
            ref_map_min = ref.min(dim=-1, keepdim=True)[0]
            ref_map_max = ref.max(dim=-1, keepdim=True)[0]
            ...
            # badNet:10; Blend:30; AdvDoor: 40; WaNet:40
            (task - 100 * distance).backward()
